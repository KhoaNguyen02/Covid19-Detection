...
class Optimize:
    ...
    def __call__(self, optim_method):
        ...
        optimization_dict = {"sgd": self.sgd(), "momentum": self.momentum(), 
                            "rms_prop": self.rms_prop(), "adam": self.adam()}
        assert optim_method in optimization_dict, "Invalid method. {} is not available".format(optim_method)
        optimization_dict[optim_method]

    def sgd(self):
        """Stochastic gradient descent optimizer."""
        for i in range(self.weights.shape[0]):
            self.weights[i] -= self.alpha * (self.dw_cache[i] + (self.lambd / self.size) * self.weights[i])
    ...
    def momentum(self):
        ...
        for i in range(self.weights.shape[0]):
            self.v_dw[i] = self.beta1 * self.v_dw[i] + (1 - self.beta1) * self.dw_cache[i]
            self.v_db[i] = self.beta1 * self.v_db[i] + (1 - self.beta1) * self.db_cache[i]
    ...
    def rms_prop(self):
        ...
        self.initialize_optimizer_params()
        for i in range(self.weights.shape[0]):
            v_dw = self.beta2 * self.v_dw[i] + (1 - self.beta2) * self.dw_cache[i] ** 2
            v_db = self.beta2 * self.v_db[i] + (1 - self.beta2) * self.db_cache[i] ** 2
    ...
    def adam(self):
        ...
        for i in range(self.weights.shape[0]):
            self.v_dw[i] = self.beta1 * self.v_dw[i] + (1 - self.beta1) * self.dw_cache[i]
            self.v_db[i] = self.beta1 * self.v_db[i] + (1 - self.beta1) * self.db_cache[i]
            v_dw_corrected = self.v_dw[i] / (1 - self.beta1 ** 2)
            v_db_corrected = self.v_db[i] / (1 - self.beta1 ** 2)
            self.weights[i] -= self.alpha * v_dw_corrected / np.sqrt(v_dw_corrected + self.epsilon)
            self.bias[i] -= self.alpha * v_db_corrected / np.sqrt(v_db_corrected + self.epsilon)
